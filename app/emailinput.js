...
const emailInput = () => {
  ...
  const handleNext = async () => {
    if (!email) {
      setNotification({ visible: true, message: 'Please enter an email', isSuccess: false });
      return;
    }
    ...
    try {
      ...
      setNotification({ visible: true, message: 'Failed to submit email. Please try again.', isSuccess: false });
    } finally {
      setLoading(false);
    }
  };
