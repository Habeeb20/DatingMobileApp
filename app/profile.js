...
const Profile = ({ navigation }) => {
  ...
  const pickImage = () => {
    ImagePicker.launchImageLibrary(
      ...
      (response) => {
        ...
        else if (response.assets && response.assets[0].uri) setProfilePicture(response.assets[0].uri);
      }
    );
  };
  ...
  const uploadImageToCloudinary = async (uri) => {
    ...
    try {
      const response = await axios.post('https://api.cloudinary.com/v1_1/dc0poqt9l/image/upload', data, {
  ...
  const handleSave = async () => {
    ...
    if (!email || !firstName.trim() || !lastName.trim() || !cloudinaryUrl || !dateOfBirth) {
    ...
    try {
      const response = await submitProfile(email, firstName, lastName, cloudinaryUrl, dateOfBirth.toISOString(), 'defaultPassword');
      console.log('Profile saved successfully:', response.data);
      if (response.data.nextStep) {
        setTimeout(() => router.push({ pathname: `/${response.data.nextStep}`, params: { email } }), 1500);
      }
    } catch (error) {
